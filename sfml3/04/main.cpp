...
{
    const sf::Vector2f headOffset = toEuclidean(float(40), arrow.rotation);
    arrow.head.setPosition(arrow.position + headOffset);
    arrow.head.setRotation(toDegrees(arrow.rotation));

    const sf::Vector2f stemOffset = toEuclidean(float(-10), arrow.rotation);
...
void initArrow(Arrow &arrow)
{
    ...
    arrow.stem.setFillColor(sf::Color::Yellow);
